"""4-Strategy Risk Parity v2 — Bug Fixes + Per-Sleeve Trend Gates

Fixes from v1:
1. Covariance window 63->42 days. Fallback = keep previous weights (not reset to 25%).
   Also compute covariance if we have at least 30 days (was: strict 63 requirement).
2. Caps widened: min 5%, max 60% (was: 10%/50%). Allow stronger conviction.
3. TLT removed from commodity universe (it's a bond, belongs in bond sleeve only).
4. Per-sleeve emergency gates: each sleeve has its own benchmark MA cross.
   - Equity: SPY MA(10) vs MA(50)
   - Commodity: DBC MA(10) vs MA(50)
   - Dividend: DVY MA(10) vs MA(50)
   - Bond: AGG MA(10) vs MA(50)
   Only the sleeve whose benchmark is in downtrend gets reduced. Others stay fully invested.
5. Emergency logging shows which sleeve triggered and post-emergency state.

Why per-sleeve gates matter:
- v1 bug: SPY downtrend reduced ALL sleeves, killing bonds during COVID
  (when bonds should have stayed fully invested = flight to safety)
- v2: March 2020 → SPY crashes, AGG rises → equity reduces, bond stays invested
- v2: 2022 → SPY down, DBC up → equity reduces, commodity stays invested

The double protection:
1. Emergency reduces the bleeding sleeve immediately
2. Next monthly rebalance, risk parity sees that sleeve was volatile ->
   reduces its weight further, increases calm sleeves

Test periods:
- Run 1: 2016-2020 (standard gate)
- Run 2: 2020-2023 (COVID + 2022 bear — the real test)
- Run 3: 2023-2025 (recent data)
Results: results_from_quant_connect/combined4stratriskparityv2/{period}/
"""
...
class Combined4StratRiskParityV2(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self.debug(
            f">>> 4-STRAT RISK PARITY v2: {len(self.equity_tickers)} stocks + "
    ...
    def on_end_of_algorithm(self):
        ...
        if self.weight_history:
            ...
            for date, w in self.weight_history:
