"""Commodity Momentum v1 — Strategy 2 (Uncorrelated to Equity v2)

Applies v2's proven scoring engine to commodity/real-asset ETFs.
The thesis: commodity momentum is structurally uncorrelated to equity
momentum. When tech drops (2022), energy/agriculture/gold often rise.
Combined with v2, this should improve portfolio Sharpe via diversification.

Design choices:
- 4 signals (no events — ETFs don't have earnings beats)
- Weights redistributed: momentum 0.40, trend 0.25, recent 0.20, vol 0.15
- 15 ETFs universe → hold top 4 (similar 25-30% concentration as v2's 50→15)
- Monthly rebalance, equal weight, same Wednesday emergency check
- NO trend gate inversion: we still reduce in SPY downtrends because
  commodity crashes happen too (COVID March 2020 = oil went negative).
  But we use downtrend_top_n=3 instead of going to cash.
- Benchmark: DBC (broad commodity index)

ETF Universe (15 instruments):
- Precious metals: GLD (gold), SLV (silver), PPLT (platinum)
- Energy: XLE (energy stocks), USO (oil), UNG (natural gas)
- Agriculture: DBA (ag commodities), MOO (agribusiness), WEAT (wheat),
  CORN (corn), SOYB (soybeans)
- Base metals: CPER (copper)
- Broad commodity: DBC (diversified), PDBC (optimized roll)
- Bonds (safe haven): TLT (20yr treasury)

Note on inception dates:
- GLD (2004), SLV (2006), DBC (2006), TLT (2002) — all fine for 2016+
- WEAT (2011), CORN (2010), SOYB (2011) — fine for 2016+
- MOO (2007), DBA (2007), USO (2006), UNG (2007) — fine
- PDBC (2014), CPER (2011), PPLT (2010) — fine
- XLE (1998) — fine
"""
...
class CommodityMomentumV1(QCAlgorithm):

    def initialize(self):
        # ── Test periods (change these per backtest run) ──
        # Run 1: 2016-2020 (standard gate, must not be terrible)
        # Run 2: 2020-2023 (commodity supercycle, should shine)
        # Run 3: 2023-2025 (recent data, validate persistence)
        # Results: results_from_quant_connect/commoditymomentumv1/{period}/
        ...
        # ── Signal weights (sum to 1.0, no events signal) ──
        ...
        self.mom_skip = 21               # skip last month (mean reversion)
        self.etf_ma_period = 50          # 50-day MA for trend signal
        self.recent_period = 21          # 21-day recent strength
        self.vol_period = 42             # 42-day volatility

        # ── 15 commodity/real-asset ETFs ──
        self.target_tickers = [
            # Precious metals
            "GLD",     # Gold
            "SLV",     # Silver
            ...
            # Energy
            "XLE",     # Energy sector stocks (Exxon, Chevron, etc.)
            "USO",     # Crude oil
            "UNG",     # Natural gas

            # Agriculture
            "DBA",     # Agriculture commodities basket
            "MOO",     # Agribusiness stocks (Nutrien, Deere, etc.)
            "WEAT",    # Wheat
            "CORN",    # Corn
            "SOYB",    # Soybeans
            ...
            "PDBC",    # Optimized commodity (better roll yield)
        ...
        self.debug(
            f">>> COMMODITY MOMENTUM v1: {len(self.target_tickers)} ETFs, "
    ...
    # ══════════════════════════════════════════════════════════════════════
    # on_data — deploy rebalance only (no event tracking for ETFs)
    # ══════════════════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════════════════
    # ETF scoring (4 signals — v2 minus events)
    # ══════════════════════════════════════════════════════════════════════

    def _score_etfs(self):
        """Score ETFs: momentum + trend + recent + volatility."""
        scores = {}
        raw_data = {}
        ...
        for ticker in self.target_tickers:
            ...
            if len(closes) >= self.etf_ma_period:
                ma_50 = np.mean(closes[-self.etf_ma_period:])
            ...
            # Signal 4: Volatility (lower = better for risk-adjusted)
        ...
        # Volatility: lower is better → rank descending
