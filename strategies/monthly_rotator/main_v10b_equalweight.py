"""Monthly Rotator v10b — Smart Equal-Weight for Any Account Size

Same scoring as v2 but with adaptive equal-weight allocation:
- Takes top 15 by score
- Removes most expensive stocks one by one until remaining all fit equal-weight
- Removes by PRICE (not score) so highest-scored affordable stocks stay
- Minimum 3 stocks always held
- At $500: holds ~4-5 stocks equal-weight
- At $10K+: holds 14-15 stocks — identical to v2

Includes IBKR portfolio sync on deploy (won't re-buy on redeploy).
...
class MonthlyRotatorV10bEqualWeight(QCAlgorithm):
    ...
    def monthly_rebalance(self):
        ...
        # Buy new positions — smart equal-weight: maximize diversity from top-ranked
        total_value = self.portfolio.total_portfolio_value
        if total_value <= 0 or n_hold <= 0:
            self.event_counts_this_month.clear()
            return

        # Step 1: collect top-ranked candidates with prices (keep rank order)
        candidates = []
        for ticker, score in ranked[:n_hold]:
        ...
        # Step 2: remove most expensive until all fit equal-weight (min 3 stocks)
        skipped = []
        while len(candidates) > 3:
            alloc = total_value / len(candidates)
            too_expensive = [c for c in candidates if c[2] > alloc]
            if not too_expensive:
                break
            most_expensive = max(too_expensive, key=lambda c: c[2])
            skipped.append(f"{most_expensive[0]}(${most_expensive[2]:.0f})")
        ...
        # Step 3: buy equal-weight across survivors
        target_alloc = total_value / len(candidates)
        bought = 0
