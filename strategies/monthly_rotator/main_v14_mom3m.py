"""Monthly Rotator v14 — Momentum Lookback A/B Test (3-month)

EXACT copy of v2 with ONE change: mom_lookback = 63 (3 months) instead of 126 (6 months).

Purpose: Test whether shorter momentum lookback captures trends earlier
and improves bull market returns. Academic literature is split — Jegadeesh
& Titman (1993) found 3-12 month momentum all work, but optimal varies
by universe and rebalance frequency.

A/B test rules:
- Compare directly to v2 on same periods (2016-2020, 2022-2023, etc.)
- Only mom_lookback changed (63 vs 126). Everything else IDENTICAL.
- If v14 wins: shorter momentum is better for our universe
- If v2 wins: 6-month lookback is right, don't touch it
"""
...
class MonthlyRotatorV14Mom3m(QCAlgorithm):
    ...
    def initialize(self):
        ...
        # ── Momentum parameters ──
        self.mom_lookback = 63            # ← CHANGED: 3 months (was 126 = 6 months)
        self.mom_skip = 21
