"""Monthly Rotator v14b — Skip Period A/B Test (5 days)

EXACT copy of v2 with ONE change: mom_skip = 5 (1 week) instead of 21 (1 month).

Purpose: Test whether shorter skip period improves momentum signal.
Academic consensus (Jegadeesh & Titman): skip the most recent 1 week
to avoid short-term reversal, but capture more of the momentum trend.
v2 skips 21 days (1 month) which may throw away too much recent information.

A/B test rules:
- Compare directly to v2 on same periods
- Only mom_skip changed (5 vs 21). Everything else IDENTICAL.
- If v14b wins: we're over-skipping and losing signal
- If v2 wins: 21-day skip is right for monthly rebalance
"""
...
class MonthlyRotatorV14bSkip5d(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self.mom_skip = 5                 # ← CHANGED: 5 days (was 21 = 1 month)
