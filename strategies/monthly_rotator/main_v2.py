"""Monthly Rotator v2 — Events as Scoring Signal

Same monthly rebalance as v1, but news events BOOST a stock's composite
score instead of triggering separate short-term trades. This captures the
event-driven edge (earnings beats, analyst upgrades) without the fee drag
of 3-5 day hold trades.

How event scoring works:
- Throughout the month, we track positive news events per stock
- At rebalance time, stocks with recent events get a scoring bonus
...
class MonthlyRotatorV2(QCAlgorithm):
