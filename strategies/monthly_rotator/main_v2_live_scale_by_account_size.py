"""Monthly Rotator v2 Live — Adaptive for any account size

Same scoring as v2 (top 15) but buys what it can afford with whole shares.
If a stock is too expensive, logs it and moves on. Naturally adapts:
- $500 account: buys 3-5 cheap stocks from the top 15
- $5K account: buys 10-12 stocks
- $10K+ account: buys all 15 (same as v2 backtest)

Includes on_data deploy trigger for mid-month deployments.
...
class MonthlyRotatorV2Live(QCAlgorithm):
    ...
    def monthly_rebalance(self):
        ...
        # Buy new positions — iterate by score (highest first), buy what we can afford
        ...
        # Sort target tickers by score (highest first) so best stocks get capital
        ...
        for ticker, score in ranked_targets:
            ...
            if target_qty < 1:
                ...
                self.current_holdings.add(ticker)  # still track it
                continue
