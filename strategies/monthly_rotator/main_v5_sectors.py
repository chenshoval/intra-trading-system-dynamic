"""Monthly Rotator v5 — Sector ETF Rotation Long-Short

Instead of picking 50 individual stocks, rotate through 11 sector ETFs.
Long top 4 sectors, short bottom 3. Same momentum+trend scoring logic.

Based on Harvey/Man Group sector analysis and Kelly/Xiu cross-sectional
momentum on portfolios. Sector-level momentum has strong academic support
and is simpler: fewer positions, very liquid, easy to short ETFs.

No event scoring (events are stock-specific, not sector-level).
"""
...
class MonthlyRotatorV5Sectors(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self.long_n = 4
        self.short_n = 3
        self.downtrend_long_n = 2
        self.downtrend_short_n = 3          # keep shorts in downtrend
        self.trend_fast = 10
        self.trend_slow = 50
        ...
        self.debug(f">>> ROTATOR v5 SECTORS: {len(self.target_tickers)} ETFs, long {self.long_n} short {self.short_n}")
    ...
    def _score_stocks(self):
        ...
        for ticker in self.target_tickers:
            ...
            raw_data[ticker] = {"momentum": momentum, "trend": trend_score, "recent": recent, "vol": vol}
    ...
    def monthly_rebalance(self):
        ...
        for ticker in list(self.long_holdings | self.short_holdings):
            if ticker not in target_longs and ticker not in target_shorts:
                symbol = self.symbols[ticker]
        ...
        # Long positions — equal weight across long_n
        long_alloc = total_value * 0.8 / n_long if n_long > 0 else 0  # 80% long
        ...
        # Short positions — equal weight across short_n
        short_alloc = total_value * 0.2 / n_short if n_short > 0 else 0  # 20% short
        ...
        self.debug(f"REBALANCE [{regime}]: long={list(target_longs)} short={list(target_shorts)}, eq=${total_value:,.0f}")
                ...
                for ticker in list(self.long_holdings):
                    if ticker not in keep_long:
                        ...
                        if self.portfolio[symbol].invested:
                            ...
                            self.total_trades += 1
                        self.long_holdings.discard(ticker)
            self.debug(f"EMERGENCY: downtrend, longs={len(self.long_holdings)} shorts={len(self.short_holdings)}")
    ...
    def on_end_of_algorithm(self):
        ...
        self.debug(f"RESULTS: Return={ret_pct:.2%} Final=${current_equity:,.0f} Trades={self.total_trades}")
